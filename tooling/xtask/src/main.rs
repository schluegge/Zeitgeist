...
mod tests {
    ...
    fn parses_zeitgeist_verify_subcommand() {
        let args = Args::try_parse_from(["cargo-xtask", "zeitgeist-verify"])
            .expect("Zeitgeist verifier subcommand should parse");
        assert!(matches!(args.command, CliCommand::ZeitgeistVerify(_)));
    }
}
