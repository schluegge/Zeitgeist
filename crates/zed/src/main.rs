...
struct Args {
    ...
    /// On macOS, the default is `~/Library/Application Support/Zeitgeist`.
    /// On Linux/FreeBSD, the default is `$XDG_DATA_HOME/zeitgeist`.
    /// On Windows, the default is `%LOCALAPPDATA%\Zeitgeist`.
